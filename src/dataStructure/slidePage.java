...
//		double wp = (double)pageWidth / Constants.DEFAULT_WIDTH;
	...
	private ArrayList<Integer> seekTitle(ArrayList<textLine> list)
	{
		 ...
		 * 3. No candidate -> in this case, title search is over and this slide will have no title.
		 */
		if(titleCandidates.size() > 0 )
		{
				...
				for(int i = 0; i < titleCandidates.get(0); i++)
				{
							...
							&& list.get(i).get_left() < 700*wp && list.get(i).get_top() >= 10*hp )
					{
						...
						if(i > 0 && list.get(i-1).get_bottom() - list.get(i).get_top() >= 0)
						{
							...
							{
								if(list.get(i).get_left() + list.get(i).get_width() + 100*wp > list.get(i-1).get_left())
		...
		if(titleCandidates.size() > 0 && titleCandidates.size() < 3)
		{
			for(int i = titleCandidates.get(titleCandidates.size() - 1) + 1; i < list.size() && titleCandidates.size() < 3; i++)
			{
				...
				if(aboveDistance < 0 && list.get(i).get_left() < 700*wp)
				{
					if(list.get(i).get_left() < list.get(j).get_left())
					{
						if(list.get(i).get_left() + list.get(i).get_width() + 100*wp > list.get(j).get_left())
							titleCandidates.add(i);
						else if(list.get(i).get_left() + list.get(i).get_width() + 200*wp > list.get(j).get_left() && list.get(i).get_height() >= (averageHeight + biggest)/2)
							titleCandidates.add(i);
					}
				...
				else if( (aboveDistance < downDistance * 0.5 || aboveDistance < downDistance - 20*hp )  && aboveDistance < 40*hp)
					if((list.get(i).get_left() >= list.get(j).get_left() && list.get(i).get_left() <= list.get(j).get_left() + list.get(j).get_width())
						|| (list.get(j).get_left() >= list.get(i).get_left() && list.get(j).get_left() <= list.get(i).get_left() + list.get(i).get_width()) )
					titleCandidates.add(i);
			}
		}
	...
	private ArrayList<Integer> seekTitleWithPTA(ArrayList<textLine> list, ArrayList<int[]> potentialTitleArea)
	{
			...
			for(int k = 0; k < potentialTitleArea.size(); k++)
			{
				...
				if(list.get(i).isSameTitlePosition(t, this._pageWidth, this._pageHeight, centered))
				{
					...
					else
					{
						...
						if(Math.min(current.get_left() + current.get_width(), list.get(i).get_left() + list.get(i).get_width()) - Math.max(current.get_left(), list.get(i).get_left()) < 0 )
						{
							titleCandidates.clear();
							titleCandidates.add(i);
						}
		...
		if(titleCandidates.size() > 0 && titleCandidates.size() < 3)
		{
			for(int i = titleCandidates.get(titleCandidates.size() - 1) + 1; i < list.size() && titleCandidates.size() < 3; i++)
			{
				...
				// Please note "inPTA" here means the current title is already in title position, no further row needed.
				boolean inPTA = false;
				for(int k = 0; k < potentialTitleArea.size(); k++)
				{
					int[] pta = potentialTitleArea.get(k);
				...
				else if( (aboveDistance < downDistance * 0.5 || aboveDistance < downDistance - 20*hp || aboveDistance <= Math.min(list.get(i).get_height(), list.get(j).get_height()))  && aboveDistance < 40*hp)
					if(Math.min(list.get(j).get_left() + list.get(j).get_width(), list.get(i).get_left() + list.get(i).get_width()) - Math.max(list.get(j).get_left(), list.get(i).get_left())
						> Math.min(list.get(j).get_width(), list.get(i).get_width()) * 0.66 )
					titleCandidates.add(i);
			}
		}
 	...
 	private void loadTextInHierarchy(ArrayList<textLine> list)
	{
		...
		for(int i = 1; i < list.size(); i++)
		{
			if(isTextContinued(list, i))
			{
				...
				if( (list.get(i).get_top() < list.get(i-1).get_top() + 7*hp && list.get(i).get_top() > list.get(i-1).get_top() - 7*hp)
					|| (list.get(i).get_bottom() < list.get(i-1).get_bottom() + 7*hp && list.get(i).get_bottom() > list.get(i-1).get_bottom() - 7*hp))
				{
					if(list.get(i).get_left() < list.get(i-1).get_left())
					{
						textLine t = list.get(i);
		...
		if(this.get_title().length() == 0)
		{
			if(list.size() > 3)
			{
				for(int i = 0; i < list.size(); i++)
				{
					if(list.get(i).get_left() > 10*wp && list.get(i).get_left() + list.get(i).get_width() < 950*wp)
					{
...
//		double hp = (double)this.get_pageHeight() / 768;
 	...
 	{
 		/*
 		 * This function is applied when a slide page has more than one sub-text-system.
 		 * If a independent sub-text-system contain too many textlines in first level,
 		 * make them as a whole with only the first textline remaining in first level.
 		 * All others' hierarchy will be plus by 1.
 		 */
 		if(list.size() < 4)
 			return list;
 	...
 	private boolean isTextContinued(ArrayList<textLine> list, int index)
	{
		...
		else
		{
			...
			else
			{
				...
				if((list.get(index).get_top() < list.get(index-1).get_top() + 7*hp && list.get(index).get_top() > list.get(index-1).get_top() - 7*hp)
					|| (list.get(index).get_bottom() < list.get(index-1).get_bottom() + 7*hp && list.get(index).get_bottom() > list.get(index-1).get_bottom() - 7*hp)
					...
					|| (list.get(index-1).get_top() - list.get(index).get_top() > 0 && list.get(index-1).get_top() - list.get(index).get_top() < list.get(index).get_height()/2))
				{
					...
					{
						if(list.get(index).get_left() + list.get(index).get_width() + 100*wp > list.get(index-1).get_left())
							return true;
					}
				}
				...
				{
					if(list.get(index).get_left() > list.get(index-1).get_left() - 90*wp && list.get(index).get_left() < list.get(index-1).get_left() - 25*wp)
						return true;
					else if(list.get(index).get_left() > list.get(index-1).get_left() + 25*wp && list.get(index).get_left() < list.get(index-1).get_left() + 50*wp)
						return true;
					else if(list.get(index).get_left() >= list.get(index-1).get_left() - 15*wp && list.get(index).get_left() <= list.get(index-1).get_left() + 15*wp)
						return true;
				}
			}
		}
...
//		double hp = (double)this.get_pageHeight() / 768;
			...
			if(list.get(index).isInSameRow(list.get(index-1)))
			{
				...
				if(list.get(index-1).get_left() < list.get(index).get_left())
				{
					if(list.get(index-1).get_left() + list.get(index-1).get_width() < 512*wp && list.get(index).get_left() > 512*wp)
					{
						if(list.get(index).get_text().charAt(0) >= 'A' && list.get(index).get_text().charAt(0) <= 'Z')
							return false;
						if(Math.abs(list.get(index-1).get_width() - list.get(index).get_width()) < Math.min(list.get(index-1).get_width(), list.get(index).get_width()))
							return false;
				...
				{
					if(list.get(index).get_left() + list.get(index).get_width() < 512*wp && list.get(index-1).get_left() > 512*wp)
					{
						if(list.get(index-1).get_text().charAt(0) >= 'A' && list.get(index-1).get_text().charAt(0) <= 'Z')
							return false;
						if(Math.abs(list.get(index-1).get_width() - list.get(index).get_width()) < Math.min(list.get(index-1).get_width(), list.get(index).get_width()))
					...
					if(list.get(index).get_left() + list.get(index).get_width() + maxGap > list.get(index-1).get_left())
						return true;
				}
			}
			else
			{
				/* for others, 6 factors to consider, quantified to a value:
				 * 1. [-10, 0] : Hierarchical gap (VETO requirement)
				...
				if(list.get(index-1).get_text().charAt(0) >= 'A' && list.get(index-1).get_text().charAt(0) <= 'Z')
				{
					...
					if(list.get(index-1).get_text().length() > 1)
						if(list.get(index-1).get_text().charAt(1) >= 'A' && list.get(index-1).get_text().charAt(1) <= 'Z')
							aboveBlockInitials = true;
				}
				else if(list.get(index-1).get_text().charAt(0) >= 'a' && list.get(index-1).get_text().charAt(0) <= 'z')
					above = 'a';
				else
				{
				...
				else if(aboveDistance >= list.get(index).get_height() * 2)
				{
					...
					{
						int above2Distance = index == 1 ? aboveDistance : list.get(index-1).get_top() - list.get(index-2).get_top();
 	...
 	private ArrayList<textOutline> refinedTextOutlineLoading(ArrayList<textLine> list, int middleLine, ArrayList<Integer> Gaps, boolean lowCaseStart, boolean extraSignStart)
 	{
 		...
 		//When there's no middle axis found, directly construct the intra-slide content tree.
 		if(middleLine < 0)
 		{
			...
			if(this.get_title().length() == 0)
			{
				if(list.size() > 3)
				{
					for(int i = 0; i < list.size(); i++)
					{
						if(list.get(i).get_left() > 10 && list.get(i).get_left() + list.get(i).get_width() < 950 && list.get(i).get_height() > 10)
						{
							this.set_title(list.get(i).get_text());
							this._titleLocation[0] = list.get(i).get_left();
							this._titleLocation[1] = list.get(i).get_left() + list.get(i).get_width();
		...
		{	//split all text-lines into 2 groups by the middle axis
			...
			if(this.get_title().length() == 0)
			{
				if(leftTexts.size() > 3 && rightTexts.size() > 0 && leftTexts.get(0).get_top() <= rightTexts.get(0).get_top())
				{
						for(int i = 0; i < leftTexts.size(); i++)
						{
							if(leftTexts.get(i).get_left() > 10 && leftTexts.get(i).get_left() + leftTexts.get(i).get_width() < 950 && leftTexts.get(i).get_height() > 10)
							{
				...
				else if(leftTexts.size() > 0 && rightTexts.size() > 3 && leftTexts.get(0).get_top() >= rightTexts.get(0).get_top())
				{
						for(int i = 0; i < rightTexts.size(); i++)
						{
							if(rightTexts.get(i).get_left() > 10 && rightTexts.get(i).get_left() + rightTexts.get(i).get_width() < 950 && rightTexts.get(i).get_height() > 10)
							{
	...
	private ArrayList<textOutline> createTextOutlines(ArrayList<textLine> list)
	{
		 ...
		 * 2. Vertically under the title
		 * 3. In left-top quarter
		 * 4. If there's multiple choices, choose the one more to the left
		 * 5. If no choice, loose 1 and 3, keep 2 for more chance */
		int temp = -1;
		for(int i = 0; i < list.size(); i++)
		{
			if(list.get(i).get_height() >= averageHeight && list.get(i).get_width() >= averageWidth/2 && list.get(i).get_top() < 350*hp
					&& list.get(i).get_top() > this._titleLocation[3] + 20*hp && list.get(i).get_left() < 450*hp)
			{
		...
		if(temp == -1)
		{
			for(int i = 0; i < list.size(); i++)
			{
				if( (list.get(i).get_height() >= (averageHeight + shortest) / 2 || list.get(i).get_height() > 15*hp) && list.get(i).get_width() >= averageWidth/2
						&& list.get(i).get_top() < 700*hp && list.get(i).get_top() > this._titleLocation[3] + 10*hp && list.get(i).get_left() < 700*wp)
				{
					if(list.get(i).get_left() <= firstLevelLeft)
					{
						firstLevelLeft = list.get(i).get_left();
		...
		for(int i = 1; i < list.size(); i++)
		{
			if(list.get(i).get_left() >= firstLevelLeft + 15*wp && list.get(i).get_left() <= firstLevelLeft + 255*wp)
				if(list.get(i).get_left() - firstLevelLeft < difference)
				{
					if(list.get(i-1).get_left() < firstLevelLeft + 15*wp && list.get(i-1).get_left() > firstLevelLeft - 25*wp)
					{
						secondLevelLeft = list.get(i).get_left();
						difference = secondLevelLeft - firstLevelLeft;
					}
					else if(list.get(i).get_left() - firstLevelLeft < 75*wp && list.get(i-1).get_left() - firstLevelLeft < 75*wp
		...
		for(int i = 1; i < list.size(); i++)
		{
			if(list.get(i).get_left() >= secondLevelLeft + 15*wp && list.get(i).get_left() <= secondLevelLeft + 255*wp)
				if(list.get(i).get_left() - secondLevelLeft < difference)
					if(list.get(i-1).get_left() < secondLevelLeft + 15*wp && list.get(i-1).get_left() > secondLevelLeft - 25*wp)
					{
		...
		for(int i = 0; i < list.size(); i++)
		{
			...
			else if(list.get(i).get_top() <= 20*hp || list.get(i).get_top() + list.get(i).get_height() < this._titleLocation[3])
			{
				textOutline t = new textOutline(list.get(i).get_text(), 0);
				result.add(t);
				last2 = last;
			...
			else
			{
				...
				int db12 = (int) (secondLevelLeft == this.get_pageWidth() ? firstLevelLeft + 50*wp : secondLevelLeft) - firstLevelLeft;
				int db23 = (int) (thirdLevelLeft == this.get_pageWidth() ? secondLevelLeft + 50*wp : thirdLevelLeft) - secondLevelLeft;
				...
				else if(( last > 0 || last2 > 0 || i == 0) && (left > firstLevelLeft - 50*wp && left < firstLevelLeft + db12/2))
				{
					textOutline t = new textOutline(list.get(i).get_text(), 1);
					result.add(t);
					last2 = last;
				...
				else if(( last > 0 || last2 > 0 || i == 0) && (left > secondLevelLeft - db12/2 && left < secondLevelLeft + db23/2))
				{
					textOutline t;
					if(last_in_system > 0)
						t = new textOutline(list.get(i).get_text(), 2);
				...
				else if(( last > 0 || last2 > 0) && (left > thirdLevelLeft - db23/2 && left < thirdLevelLeft + 100*wp))
				{
					textOutline t;
					if(last_in_system > 1)
						t = new textOutline(list.get(i).get_text(), 3);
	...
	private ArrayList<textOutline> createTextOutlinesBySelfStats(ArrayList<textLine> list)
	{
		...
		for(int i = 0; i < list.size(); i++)
		{
			...
			{
				if(stats.get(j).get_left() - 5*wp < list.get(i).get_left() && stats.get(j).get_left() + 5*wp > list.get(i).get_left())
				{
					match = true;
		...
		while(previous)
		{
			...
			for(int i = 1; i < list.size(); i++)
			{
				if(list.get(i).get_left() > firstLevelLeft - 5*wp && list.get(i).get_left() < firstLevelLeft + 5*wp)
				{
					if(list.get(i).get_left() - list.get(i-1).get_left() > 5*wp	&& list.get(i).get_left() - list.get(i-1).get_left() < 75*wp
		...
		for(int i = 0; i < list.size(); i++)
		{
			...
			else
			{
				int left = list.get(i).get_left();
				int db12 = (int) (secondLevelLeft == this.get_pageWidth() ? firstLevelLeft + 50*wp : secondLevelLeft) - firstLevelLeft;
				int db23 = (int) (thirdLevelLeft == this.get_pageWidth() ? secondLevelLeft + 80*wp : thirdLevelLeft) - secondLevelLeft;
				if(left >= firstLevelLeft - 5*wp && left <= firstLevelLeft + 5*wp)
				{
					textOutline t = new textOutline(list.get(i).get_text(), 1);
					result.add(t);
	...
	public boolean isSamePage(slidePage s)
	{
		...
		else
		{
			...
			else if (lr <= 0.4 && wr >= 0.6)
			{
				...
				int tl = this.get_title().length() > s.get_title().length() ? this.get_title().length() : s.get_title().length();
		...
		if(biggestLineSpace > aveHeight * 4)
		{
			...
			for(int i = 1; i < upperPart.size(); i++)
				if(Math.min(upperPart.get(i).get_left() + upperPart.get(i).get_width(), upperPart.get(i-1).get_left() + upperPart.get(i-1).get_width())
					- Math.max(upperPart.get(i).get_left(), upperPart.get(i-1).get_left()) > Math.min(upperPart.get(i).get_width(), upperPart.get(i-1).get_width()) * 0.66)
					covered++;
			...
			for(int i = 1; i < bottomPart.size(); i++)
				if(Math.min(bottomPart.get(i).get_left() + bottomPart.get(i).get_width(), bottomPart.get(i-1).get_left() + bottomPart.get(i-1).get_width())
					- Math.max(bottomPart.get(i).get_left(), bottomPart.get(i-1).get_left()) > Math.min(bottomPart.get(i).get_width(), bottomPart.get(i-1).get_width()) * 0.66)
					covered++;
	...
	private ArrayList<int[]> detectTable(ArrayList<textLine> list, double wp, double hp, String lectureID) throws IOException
	{
		...
		class tableColumn{
			...
			public tableColumn combineColumn(tableColumn that, ArrayList<textLine> list)
			{
				...
				{
					if(list.get(c1.getColumnCandidates().get(x)).get_top() < list.get(c2.getColumnCandidates().get(y)).get_top())
					{
					...
					else if(list.get(c1.getColumnCandidates().get(x)).get_top() > list.get(c2.getColumnCandidates().get(y)).get_top())
					{
		 ...
		 * 2. If one textline-pair is in the same column, check whether one of the textlines
		 *    is already in an existing tableColumn, taking 'k' and 'l' to do this loop.
		 * 3. If accidently both textlines in the target pair are in the existing tableColumn, pass.
								 ...
								 || (alignResult[2] & potentialTableColumn.get(k).getAlign()[2]))
								{
									...
									if(!done)
									{
										if(list.get(j).get_height() < potentialTableColumn.get(k).getAveHeight() * 1.5
												&& list.get(j).get_height() > potentialTableColumn.get(k).getAveHeight() * 0.67)
											{
												int newAveHeight = potentialTableColumn.get(k).getAveHeight() * potentialTableColumn.get(k).getColumnCandidates().size();
												newAveHeight = (newAveHeight + list.get(j).get_height()) / (potentialTableColumn.get(k).getColumnCandidates().size() + 1);
												potentialTableColumn.get(k).setAveHeight(newAveHeight);
												potentialTableColumn.get(k).getColumnCandidates().add(j);
												for(int x = 0; x < 3; x++)
													potentialTableColumn.get(k).getAlign()[x] = alignResult[x] & potentialTableColumn.get(k).getAlign()[x];
				...
				if(potentialTableColumn.get(j).getRightEdge() > potentialTableColumn.get(h).getLeftEdge())
				{
					...
					for(int k = 0; k < potentialTableColumn.get(j).getColumnCandidates().size(); k++)
					{
						for(int l = 0; l < potentialTableColumn.get(h).getColumnCandidates().size(); l++)
						{
							if(potentialTableColumn.get(j).getColumnCandidates().get(k) == potentialTableColumn.get(h).getColumnCandidates().get(l))
							{
								...
								else if( (j > 0 && potentialTableColumn.get(h).getLeftEdge() > potentialTableColumn.get(j-1).getRightEdge())
									  || (h < potentialTableColumn.size()-1 && potentialTableColumn.get(j).getRightEdge() < potentialTableColumn.get(h+1).getLeftEdge()))
								{
									share = true;
									break;
								}
								...
								{
									int less = potentialTableColumn.get(j).getColumnCandidates().size() >= potentialTableColumn.get(h).getColumnCandidates().size() ? h : j;
					...
					if(covered > Math.min(potentialTableColumn.get(j).getColumnWidth(), potentialTableColumn.get(h).getColumnWidth()) * 0.66)
					{
						if( (potentialTableColumn.get(j).getAlign()[0] & potentialTableColumn.get(h).getAlign()[0])
						 ...
						 || (potentialTableColumn.get(j).getAlign()[2] & potentialTableColumn.get(h).getAlign()[2]) )
						{
							if( (j > 0 && potentialTableColumn.get(h).getLeftEdge() > potentialTableColumn.get(j-1).getRightEdge())
							 || (h < potentialTableColumn.size()-1 && potentialTableColumn.get(j).getRightEdge() < potentialTableColumn.get(h+1).getLeftEdge()))
							{
								if(atop - btop < 0)
								{
									int distance = btop - abottom;
									if(distance < list.get(potentialTableColumn.get(j).getColumnCandidates().get(0)).get_height() * 2)
										if(Math.max(potentialTableColumn.get(h).getAveHeight(), potentialTableColumn.get(j).getAveHeight()) <
		...
		for(int i = 0; i < potentialTableRow.size(); i++)
			for(int j = 0; j < potentialTableColumn.size(); j++)
			{
				for(int k = 0; k < potentialTableRow.get(i).size(); k++)
					...
					{
						if(potentialTableRow.get(i).get(k) == potentialTableColumn.get(j).getColumnCandidates().get(l))
						{
		...
		for(int i = 0; i < crossedItemsInfo.size(); i++)
		{
			...
			for(int j = 0; j < supposedTableItemsGroup.size(); j++)
			{
				...
				{
					if(currentItem[1] == supposedTableItemsGroup.get(j).get(k)[1] || currentItem[2] == supposedTableItemsGroup.get(j).get(k)[2])
					{
		 ...
		 * when combining, using 'Merge sort', ordered by vertical coordinate
		 */
				...
				for(int j = i + 1; j < supposedTableItemsGroup.size(); j++)
				{
					...
					for(int k = 0; k < supposedTableItemsGroup.get(i).size(); k++)
					{
						for(int l = 0; l < supposedTableItemsGroup.get(j).size(); l++)
						{
							...
							{
								ArrayList<int[]> newGroup = new ArrayList<int[]>();
								for(int x = 0, y = 0; x < supposedTableItemsGroup.get(i).size() && y < supposedTableItemsGroup.get(j).size(); )
								{
									if(supposedTableItemsGroup.get(i).get(x)[0] < supposedTableItemsGroup.get(j).get(y)[0])
									{
									...
									else if(supposedTableItemsGroup.get(i).get(x)[0] > supposedTableItemsGroup.get(j).get(y)[0])
									{
		...
		for(int x = 0; x < supposedTableItemsGroup.size(); x++)
		{
			...
			for(int i = 0; i < potentialTableRow.size(); i++)
			{
				...
				{
					for(int k = 0; k < supposedTableItemsGroup.get(x).size(); k++)
						if(potentialTableRow.get(i).get(j) == supposedTableItemsGroup.get(x).get(k)[0])
						{
				...
				if(list.get(j).isInside(tableEdge))
				{
					...
					else if(rowNum >= 0)
					{
						...
						for(int k = 0; k < potentialTableColumn.size(); k++)
						{
							...
							{
								if((k > 0 ? newColumn.getLeftEdge() > potentialTableColumn.get(k-1).getRightEdge() : true)
										&& newColumn.getRightEdge() < potentialTableColumn.get(k).getLeftEdge())
								{
									...
									&& newColumn.getRightEdge() - potentialTableColumn.get(k).getLeftEdge() > Math.min(newColumn.getColumnWidth(), potentialTableColumn.get(k).getColumnWidth() * 0.5))
								{
									boolean seperate = false;
									for(int l = 0; l < potentialTableColumn.get(k).getColumnCandidates().size(); l++)
										if(list.get(j).isInSameRow(list.get(potentialTableColumn.get(k).getColumnCandidates().get(l))))
										{
											seperate = true;
											break;
										}
									...
									&& potentialTableColumn.get(k-1).getRightEdge() - newColumn.getLeftEdge() > Math.min(newColumn.getColumnWidth(), potentialTableColumn.get(k-1).getColumnWidth() * 0.5))
								{
									boolean seperate = false;
									for(int l = 0; l < potentialTableColumn.get(k-1).getColumnCandidates().size(); l++)
										if(list.get(j).isInSameRow(list.get(potentialTableColumn.get(k-1).getColumnCandidates().get(l))))
										{
											seperate = true;
											break;
										}
		...
		*/
		...
		for(int i = 0; i < supposedTableItemsGroup.size(); i++)
		{
			...
			{
				if(potentialTableColumn.get(allColumn.get(j)).getRightEdge() > potentialTableColumn.get(allColumn.get(j+1)).getLeftEdge())
					continue;
				else
				{
					boolean overlap = false;
					for(int k = 0; k < j; k++)
						if(potentialTableColumn.get(allColumn.get(k)).getRightEdge() > potentialTableColumn.get(allColumn.get(j)).getRightEdge())
							overlap = true;
					if(overlap)
						continue;
			...
			if(allColumn.size() == 2)
			{
				if(potentialTableColumn.get(allColumn.get(0)).getRightEdge() < potentialTableColumn.get(allColumn.get(1)).getLeftEdge())
				{
					if(potentialTableColumn.get(allColumn.get(0)).getRightEdge() < this._pageWidth * 0.55 && potentialTableColumn.get(allColumn.get(1)).getLeftEdge() > this._pageWidth * 0.45)
					{
						twoColumnTextDeduction = 1;
						if(potentialTableColumn.size() == 2)
							twoColumnTextDeduction += 1;
						if(potentialTableColumn.get(allColumn.get(0)).getColumnCandidates().size() + potentialTableColumn.get(allColumn.get(1)).getColumnCandidates().size() >= list.size() * 0.8)
							twoColumnTextDeduction += 1;
						if(columnBonus > 1)
							twoColumnTextDeduction = twoColumnTextDeduction - 2 < 0 ? 0 : twoColumnTextDeduction - 2;
						if(potentialTableColumn.get(allColumn.get(0)).getColumnCandidates().size() + potentialTableColumn.get(allColumn.get(1)).getColumnCandidates().size() == supposedTableItemsGroup.get(i).size())
		 ...
		 * Next, check all free textlines (not the crossed item in the group), if its horizontal or vertical distance to the initial table area
		 *       is acceptable (1.25 * columnGap and 1.5 * rowHeight), expand the table area by this new table item.
		 * Repeat last step until the table area hasn't changed anymore.
		 * Moreover, search the renewed area to check whether there's "half-involved" text-line, if so, expand to involve all.
		 * Finally, search for the potential header line (Don't need to be aligned, but close and not too wide)
		 */
		for(int i = 0; i < supposedTableItemsGroup.size(); i++)
		{
			ArrayList<Integer> allColumn = new ArrayList<Integer>();
			...
			while(!stable)
			{
				...
				for(int j = 0; j < allColumn.size(); j++)
				{
					for(int k = 0; k < potentialTableColumn.get(allColumn.get(j)).getColumnCandidates().size(); k++)
					{
						textLine current = list.get(potentialTableColumn.get(allColumn.get(j)).getColumnCandidates().get(k));
						if(!current.isInside(tableEdge))
						{
							if(current.isAwayFrom(tableEdge))
							{
								...
								{
									if(j > 0 && current.get_left() < potentialTableColumn.get(allColumn.get(j-1)).getRightEdge())
										bonusRate = 0.25;
			...
			/*
			//here starts the header check
			for(int j = 0; j < list.size(); j++)
			{
				if(list.get(j).get_bottom() > tableEdge[2])
					break;
				else
				{
		...
		for(int i = 0; i < potentialTableRow.size(); i++)
		{
			...
			for(int j = 0; j < potentialTableRow.get(i).size(); j++)
			{
				for(int k = 0; k < crossedItemsInfo.size(); k++)
					if(potentialTableRow.get(i).get(j) == crossedItemsInfo.get(k)[0])
					{
						System.out.print(list.get(potentialTableRow.get(i).get(j)).get_text() +'\t' + "||" + '\t');
						break;
					}
