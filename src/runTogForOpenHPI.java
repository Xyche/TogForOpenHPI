...
public class runTogForOpenHPI {
	...
	public static void main(String[] args) throws SQLException, ParserConfigurationException, SAXException, IOException,
		...
		if (havePPTX)
			finalResults = og.generateOutlineWithPPTX(tll,
					Constants.joinPath(workingFolder, lecture_id, Constants.DEFAULT_SLIDES_PPTX));
		else if (havePDF) {
			ArrayList<textLine> tll_pdf = new ArrayList<textLine>();
			tll_pdf = loadOcrResults(OCROriginMode.PDF, workingFolder, lecture_id, localTimeZoneOffset,
					changeBBImageNames);
		...
		while (havingAdaptiveDifference && roundNum <= 3) {
			// Recode the adaptive status in the last round, for future
			// comparison
			...
			og.set_topicParams(textOutline.count(finalResults), lastRoundBeginningDot);
			...
			if (og.get_potentialTitleArea().size() != lastRoundTableAreas.size()) {
				...
				for (int i = 0; i < og.get_potentialTitleArea().size(); i++) {
					...
					for (int j = 0; j < lastRoundTableAreas.size(); j++) {
						...
						if (Math.abs(current[0] - ref[0]) <= pageWidth / 256
								&& Math.abs(current[1] - ref[1]) <= pageHeight / 256 && current[2] == ref[2]
								&& Math.abs(current[3] - ref[3]) <= pageHeight / 256) {
							match = true;
							break;
						}
					}
		...
		/*
		 * LoggerSingleton.info(); LoggerSingleton.info(
		 * "------------------ADAPTIVE ROUND---------------------------");
		 * LoggerSingleton.info();
		 * 
		 * tll = loadOcrResults(OCR_Origin, lecture_id, localTimeZoneOffset,
		 * false); finalResults = og.generateOutline(tll);
		 */
		/* show result */
		...
		for (int i = 0; i < finalResults.size(); i++) {
			...
			output.append(finalResults.get(i).get_hierarchy() + "\t" + finalResults.get(i).get_child() + "\t"
					+ finalResults.get(i).get_text());
			output.newLine();
			output.close();
	...
	public static ArrayList<textOutline> autoSegmentationAndAnnotation(ArrayList<textOutline> fr, String workingFolder,
		...
		for (int i = 0; i < onlyTitles.size(); i++) {
			if (i == onlyTitles.size() - 1)
				...
				int t = (int) (onlyTitles.get(i + 1).get_time().getTime() - onlyTitles.get(i).get_time().getTime())
						/ 1000;
		...
		// int totalTime = (int)
		// (onlyTitles.get(onlyTitles.size()-1).get_time().getTime() -
		// onlyTitles.get(0).get_time().getTime()) / 1000 + 30;
		int totalTime = 0;
		...
		if (onlyTitles.isEmpty())
			...
			totalTime = (int) (onlyTitles.get(onlyTitles.size() - 1).get_time().getTime()
					- onlyTitles.get(0).get_time().getTime()) / 1000 + 30;

		if (haveSegment) {
			averageSegLength = Math.max(averageSegLength, onlyTitles.isEmpty() ? 0 : totalTime / onlyTitles.size());

			// averageSegLength = averageSegLength > totalTime/5 ?
			// averageSegLength : totalTime/5;
			...
			// !!!! INF loop, if first duration is longer, than the
			// averageSegLength
		...
		for (int i = 0; i < onlyTitles.size(); i++) {
			...
			if (onlyTitles.get(i).get_childEnd() >= 0) {
				...
				LoggerSingleton.info("< Segment '" + onlyTitles.get(longestPos).get_text() + "' > Duration: "
						+ ai.secondsToTime(onlyTitles.get(i).get_childEnd()));
